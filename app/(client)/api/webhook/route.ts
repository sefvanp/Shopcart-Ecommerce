...
export async function POST(req: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      console.log("✅ Order successfully created in Sanity ID:", order._id);
    } catch (error) {
...
async function createOrderInSanity(
  ...
  try {
    ...
    console.error("⚠️ Address JSON parse error:", e);
  }
  ...
  for (const item of lineItemsWithProduct.data) {
    ...
    if (!productId) {
      console.warn("⚠️ Warning: Product ID missing in Stripe metadata for item:", item.id);
      continue;
    }
  ...
  console.log("🚀 Sending document to Sanity backendClient...");
